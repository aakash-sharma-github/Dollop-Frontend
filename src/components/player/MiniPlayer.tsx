...
const PlayIcon = ({ color }: { color: string }) => (
  <Svg width={20} height={20} viewBox="0 0 24 24" fill={color}><Path d="M6 4.75L19.5 12L6 19.25V4.75Z" /></Svg>
);
const PauseIcon = ({ color }: { color: string }) => (
  <Svg width={20} height={20} viewBox="0 0 24 24" fill={color}><Rect x="5" y="4" width="4" height="16" rx="1" /><Rect x="15" y="4" width="4" height="16" rx="1" /></Svg>
);
const NextIcon = ({ color }: { color: string }) => (
  <Svg width={20} height={20} viewBox="0 0 24 24" fill={color}><Path d="M5 4.75L16 12L5 19.25V4.75Z" /><Rect x="17" y="4" width="2.5" height="16" rx="1" /></Svg>
);
...
export function MiniPlayer(): React.JSX.Element | null {
  ...
  const { currentSong, isPlaying, isLoading, positionMs, durationMs, isVisible } = usePlayerStore();
  ...
  return (
    ...
    >
      <Pressable
        style={[styles.container, { backgroundColor: colors.miniPlayerBackground, borderColor: colors.border }]}
        onPress={() => navigation.navigate(ROUTES.PLAYER)}
      >
        {/* Progress bar — top edge */}
        <View style={[styles.progressTrack, { backgroundColor: colors.border }]}>
          <View style={[styles.progressFill, { backgroundColor: colors.brand, width: `${progress * 100}%` }]} />
        </View>

        <View style={styles.content}>
          {/* Artwork */}
          <View style={[styles.artwork, { backgroundColor: artworkBg }]}>
            <Text style={styles.artworkLetter}>{currentSong.title.charAt(0).toUpperCase()}</Text>
          </View>
